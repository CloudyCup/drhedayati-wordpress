...
| File · class | Responsibility |
|---|---|
| `class-admin-access.php` · `Hedayati_Admin_Access` | `admin_init` (priority 1) redirect of an *interactive* wp-admin page view by a non-admin routed role → `workspace_url_for()` (`/panel/` for teacher/TA/reception/manager, `/account/` for student, `''` for the administrator). `is_interactive_admin_request()` excludes `wp_doing_ajax()`/`wp_doing_cron()`/`WP_CLI`/`REST_REQUEST` and `pagenow` ∈ {`admin-post.php`,`admin-ajax.php`,`async-upload.php`}, then requires `is_admin()`. D54 removed the `profile.php` exemption once `Hedayati_Panel_Security`/`Hedayati_Student_Portal` gave every role its own front-end password screen. `wp_safe_redirect` + `exit`, `nocache_headers()` first. `show_admin_bar` forced off for every non-admin routed role. **Fully enforced:** `ENFORCED_ROLES` = student + teacher + teacher_assistant + reception + hedayati_manager; `enforced_roles()` applies the `hedayati_admin_redirect_roles` filter; `is_enforced_for()` requires *all* of a user's roles to be in that set and bails for `manage_options`. |
