...
| 10.4 | Manager: courses, runs, assignments, enrollments, verification, private documents, settings, audit logs | 🟡 Core operations implemented. On `feature/manager-experience` (D45), `/panel/` is the primary manager UX: AI-Studio-inspired home with live non-sensitive KPIs, plus **in-panel course management** (`?view=courses`) and **featured curation** (`?view=featured`) against the real course CPT — search, featured filter, and nonce + `edit_post`-guarded feature/publish toggles with a server-side 8-slot cap. Per-field course editing, academic operations, settings, teachers and the audit viewer still open their existing secure WordPress controllers (wp-admin is now a fallback, not the primary surface). Prototype-only requests/certificates/progress/materials/tickets/notifications remain planned modules with open owner decisions — `docs/AI_STUDIO_PANEL_MATRIX.md` §E. |
