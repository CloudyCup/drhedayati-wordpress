...
| 6.11 | Branded login / password-reset UI + staff account creation | ✅ (code) on `feature/manager-experience` (D53 Phase F, plugin 1.14.0, not merged/not staging-tested): a dedicated front-end **`/login/`** (`Hedayati_Login` + `page-login.php` + `auth.css`) in the public-site design language — login / forgot-password / reset-link landing / new-password / check-email / password-changed, all in one shell. Drives WordPress primitives only: `wp_signon()` (full `authenticate` chain — phone-or-username, rate limiter, generic error), core `retrieve_password()` (enumeration-safe, result never inspected), core `check_password_reset_key()` + `reset_password()` (token untouched, email link only re-pointed). `redirect_to` via `wp_validate_redirect()`. `wp-login.php` bounced for normal visitors, intact for the admin. Forced first-login change (`Hedayati_Account_Security`) already a front-end flow. **No public registration — deliberate** (D41). |
...
| 8.6 | Verification does not gate certificates. Certificate issuance is an explicit, manual staff action per enrollment (D48) — never auto-granted by verification or run completion. | ✅ resolved by D48: `hedayati_manage_certificates` (manager/admin) issues; `UNIQUE(enrollment_id)`; public `/verify/` shows minimal non-PII fields only. |
...
| 8.8 | Student portal presents a responsive Persian-first learning dashboard and shows upcoming classes only from the signed-in student's active enrollments | 🟡 Implemented on `feature/manager-experience`: service-derived, read-only schedule excludes other students, inactive enrollments, completed/cancelled runs, and past sessions; static/runtime/browser reviewed, not merged or staging-tested |
...
| 10.4 | Manager: courses, runs, assignments, enrollments, verification, private documents, settings, audit logs, **consultations, certificates, materials, support, notifications** | ✅ for what's built, on `feature/manager-experience` (D45–D53, plugin 1.14.0): `/panel/` is the sole manager UX. **Fully in-panel now:** dashboard (real KPIs), courses + featured, **course create/edit (`?view=course-new`/`course-edit`)**, **academic operations (`?view=academic` — runs / staff / sessions / enrollments / attendance / public opt-in)**, students + **verification / national-ID reveal / private documents (`?view=students`)**, teachers (`?view=teachers`), audit (`?view=audit`), settings, consultations, certificates, materials, support, notifications. No wp-admin link remains. Node 940/0, Docker CI GREEN (run `34161338173`, 686/0). Not browser-reviewed / not merged / not staging-tested. |
| 10.13 | **wp-admin is administrator-only; non-admin roles use the front-end panel/account (D53)** | ✅ (code) on `feature/manager-experience` (D53, plugin 1.14.0): `Hedayati_Admin_Access` redirects interactive wp-admin → `/panel/` / `/account/` and hides the admin bar for **every** non-administrator Hedayati role (student + teacher + teacher_assistant + reception + hedayati_manager). Transport endpoints (`admin-post.php`/`admin-ajax.php`/`async-upload.php`/REST/cron/WP-CLI) + `profile.php` preserved; no capability revoked, no `map_meta_cap` filter, no global wp-admin disable. Administrator keeps full wp-admin. Static + Docker CI verified (D53.A–G); the `admin_init` redirect firing in a real browser is a staging acceptance item. |
| 10.7 | Public consultation request workflow (form → staff queue → status) | ✅ D46 — `hedayati_consultations`, nonce + honeypot + per-IP rate limit, E.164 phone; `hedayati_manage_consultations` queue. Not staging-tested. |
| 10.8 | Objective student progress + attendance (no invented percentages) | ✅ D47 — `Hedayati_Progress_Service`: run progress (held/total sessions) and attendance rate (present/recorded) shown separately; zero-session → "—". |
| 10.9 | Certificates + public verification | ✅ D48 — see 8.6. |
| 10.10 | Course/session materials (link / note / file) with enrollment-scoped access | ✅ D49 — `hedayati_session_materials`, private file store separate from identity docs, nonced enrollment-checked download. |
| 10.11 | Internal support tickets (student ↔ staff), IDOR-safe | ✅ D51 — `hedayati_support_tickets` + `_messages`, ownership on every read/write, 4 statuses. |
| 10.12 | Internal on-site notifications (no email/SMS) | ✅ D50 — `hedayati_notifications`, deliberate event set, owner-scoped read state. |
| 10.5 | Audit-log viewer | 🟡→✅ in-panel (D53): `/panel/?view=audit` (`Hedayati_Audit_Panel`), `hedayati_view_audit_logs`, object-type/action/actor filters + pagination, metadata-only (no IP/UA). The wp-admin viewer under «عملیات آموزشی → گزارش رویدادها» remains as the administrator fallback. Richer UX (export, date range) still future. |
