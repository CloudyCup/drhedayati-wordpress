...
| 10.4 | Manager: courses, runs, assignments, enrollments, verification, private documents, settings, audit logs | 🟡 Core operations are implemented. On `feature/manager-experience`, `/panel/` now provides an AI-Studio-inspired manager home with live non-sensitive KPIs and capability-gated navigation to every implemented operation; mature edit/detail workflows still open their existing secure WordPress controllers. Prototype-only requests/certificates/progress/tickets/notifications remain planned modules (D44). |
