...
| 10.4 | Manager: courses, runs, assignments, enrollments, verification, private documents, settings, audit logs, **consultations, certificates, materials, support, notifications** | 🟡 `feature/manager-experience` (D45–D52): `/panel/` is the primary manager UX with a capability-gated module registry. In-panel: dashboard (real KPIs), courses + featured (`?view=courses`/`?view=featured`), consultations queue (`?view=consultations`), certificates issue/revoke (`?view=certificates`), support queue (`?view=support`), institute settings (`?view=settings`), materials (in the run view). Academic operations, teachers, per-field course editing and the audit viewer still open their existing secure WordPress controllers (wp-admin = fallback). Node 876/0, Docker 576/0 PASS. Not browser-reviewed / not merged / not staging-tested. |
