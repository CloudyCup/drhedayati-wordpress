...
| 10.4 | Manager: courses, runs, assignments, enrollments, verification, private documents, settings, audit logs, **consultations, certificates, materials, support, notifications** | 🟡 `feature/manager-experience` (D45–D53): `/panel/` is the primary manager UX with a capability-gated module registry. In-panel: dashboard (real KPIs), courses + featured, consultations, certificates, support, institute settings, materials, **teachers (`?view=teachers`, D53 — full CRUD over the `teacher` CPT)**, **audit (`?view=audit`, D53 — read-only, paginated, metadata-only)**. Still wp-admin, pending **Phase C/E**: per-field course create/edit, academic operations (`hedayati-academic`), verification queue / private-document review (`hedayati-students`) — manager-only interim links tagged «موقت». Node 953/0. Docker CI GREEN (run `34154472653`, 623/0). Not browser-reviewed / not merged / not staging-tested. |
| 10.13 | **wp-admin is administrator-only; non-admin roles use the front-end panel/account (D53)** | 🟡 `feature/manager-experience` (D53): `Hedayati_Admin_Access` redirects interactive wp-admin → `/panel/` / `/account/` and hides the admin bar for non-admin routed roles; transport endpoints (`admin-post.php`/`admin-ajax.php`/`async-upload.php`/REST/cron/WP-CLI) preserved; no capability revoked. **Staged:** enforced for student/teacher/teacher_assistant now; reception/manager gated on the Phase E front-end port (one-line `hedayati_admin_redirect_roles` filter flip). Administrator keeps full wp-admin. Static 75/0; browser redirect = staging acceptance item. |
